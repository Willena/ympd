...
int callback_mpd(struct mg_connection *c, struct threads_status *status) {
    ...
    switch (cmd_id) {
        case PLG_RELOAD:
            printf("(Callback) -> reload plugin_list asked !\n");
            load_plugins(status);
            break;
        case YMPD_PLG:
            ympd_run_plugin_cmd(c, status);
            break;
...
void ympd_run_plugin_cmd(struct mg_connection *c, struct threads_status *status)
{
    int pos = strchr(c->content, ',')  - c->content + 1;
